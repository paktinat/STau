...
inline void printTwikiTable(  vector<TString> samplesName, map<TString, TString> cutLabels, vector<TString> orderedCutLabels, map<TString, map<TString, TH1F*> > counterH, bool detailed=false ){
  ...
  for( vector<TString>::const_iterator c_i = orderedCutLabels.begin(); c_i != orderedCutLabels.end(); c_i++ ){
    ...
    for( vector<TString>::const_iterator s_i=samplesName.begin(); s_i != samplesName.end(); s_i++){
      ...
      if(mySample=="MC" || mySample=="HT-Data" || detailed){
	double err;
	double integral = counterH[myCut][ mySample]->IntegralAndError(1 ,counterH[myCut][ mySample]->GetNbinsX() , err);
	sprintf( tmp, " | %.2f +- %.2f", integral, err );
      }
...
inline float getMistagSF(float &err, TString tagger, float pt, float eta){//NEW
