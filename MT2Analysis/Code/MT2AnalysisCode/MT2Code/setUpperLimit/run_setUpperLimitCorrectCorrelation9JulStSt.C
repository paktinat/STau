...
run_setUpperLimitCorrectCorrelation9JulStSt() {

    std::vector< TString > sin;
    int argc = gApplication->Argc();
    for (int i = 0; i < argc; i++) {
        TString argvi = gApplication->Argv(i);
        if (argvi.Contains("run_setUpperLimitCorrectCorrelation9JulStSt.C"))
    ...
    TH2D* htmp = (TH2D*) TFile::Open("referenceXSecs.root")->Get("StSt_8TeV_NLONLL_LSP"); 
