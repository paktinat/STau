...
def parseInputFile(name):   
  ...
  if os.path.exists(stringInfo[5]) == False:
	  os.system("mkdir -p " + stringInfo[5]) 
  else:
	  print "WorkPath", stringInfo[5], "exists, exiting"
	  sys.exit(-1)
...
def process(task, conf):
  ...
  # for one STOP sample changed index1 to 18 or change nameOfFolder
