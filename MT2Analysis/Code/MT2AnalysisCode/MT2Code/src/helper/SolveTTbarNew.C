...
*/
   // do the minimization
   min->Minimize(); 
//   unsigned int nsteps = 600;
//   TGraph *gr = new TGraph(nsteps);
//   double *h1 = gr->GetX(); double *h2 = gr->GetY();
//   min->Scan(1,nsteps,&(*h1),&(*h2)/*,-8000.,8000.*/); 
//   if(nsteps!=600) cout << "nsteps " << nsteps << endl;
//   min->Scan(0,nsteps,&(*h1),&(*h2)/*,-8000.,8000.*/); 
   statuss = min->Status();
