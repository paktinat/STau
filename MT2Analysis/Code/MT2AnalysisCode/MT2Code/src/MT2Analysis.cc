...
void MT2Analysis::Begin(const char* filename){
	...
	if (fisData )
     ...
     myFilter = new EventFilterFromListStandAlone(GETDATALOCALPATH(MT2Analysis/Code/MT2AnalysisCode/MT2Code/data/HCALLaser2012AllDatasets.txt.gz));
...
void MT2Analysis::GetLeptonJetIndices(){
	...
	for(int ij=0; ij < (fisCHSJets?fTR->PFCHSNJets:fTR->NJets); ++ij){
		...
		for (int elIndex=0; elIndex<fElecs.size(); ++elIndex){
		  if(!IsGoodMT2ElectronSelIDforStop(fElecs[elIndex]))
		    continue;
		...
		for (int muIndex=0; muIndex<fMuons.size(); ++muIndex){
		  ...
		  if(!IsGoodMT2MuonStop(fMuons[muIndex]))
		    continue;
		  TLorentzVector mu;
